  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      const url =
        ...
        "&lon=" +
        lon +
        "&appid=3b5380aeb364a68d8896f38f7d1bdcea";
...
function weatherReport(data) {
  const urlcast =
    ...
    data.name +
    "&appid=4cdb8227ca603e4fe167a816e3c864d6";
...
function hourForecast(forecast) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    templist +=
      ' <div class="next"> <div><p class="time">' +
...
function dayForecast(forecast) {
  ...
  for (let i = 8; i < forecast.list.length; i += 8) {
    weekF +=
      ' <div class="dayF col-lg-3"><p class="date">' +
      new Date(forecast.list[i].dt * 1000)
